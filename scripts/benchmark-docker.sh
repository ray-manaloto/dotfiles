...
# #673: the target platform is one parameter. The triple->uname mapping
# lives in python (zero-bash-logic); these are assignments, not logic.
PLATFORM="$(uv run --project python dotfiles-setup platform triple)"
...
STARTUP_TIME=$(time_cmd "docker run --rm --platform ${PLATFORM} ${IMAGE_REF} true")
echo "Startup: ${STARTUP_TIME}s"

# F4: AMD64 correctness
echo "--- F4: AMD64 Correctness ---"
UNAME_ARCH="$(docker run --rm --platform "${PLATFORM}" "${IMAGE_REF}" uname -m 2>/dev/null || echo 'FAILED')"
echo "uname -m: ${UNAME_ARCH}"
AMD64_CORRECT="false"
if [ "$UNAME_ARCH" = "${EXPECTED_MACHINE}" ]; then
    AMD64_CORRECT="true"
...
FS_WRITE_TIME=$(time_cmd "docker run --rm --platform ${PLATFORM} ${IMAGE_REF} bash -c 'dd if=/dev/zero of=/tmp/testfile bs=1M count=100 2>/dev/null'")
FS_READ_TIME=$(time_cmd "docker run --rm --platform ${PLATFORM} ${IMAGE_REF} bash -c 'dd if=/dev/zero of=/tmp/testfile bs=1M count=100 2>/dev/null && dd if=/tmp/testfile of=/dev/null bs=1M 2>/dev/null'")
echo "Write 100MB: ${FS_WRITE_TIME}s, Read 100MB: ${FS_READ_TIME}s"

# F6: Memory usage (idle container)
echo "--- F6: Memory Usage ---"
CONTAINER_ID=$(docker run -d --rm --platform "${PLATFORM}" "${IMAGE_REF}" sleep 30)
sleep 2
...
TOOLS_CHECK="$(docker run --rm --platform "${PLATFORM}" "${IMAGE_REF}" bash -lc 'mise ls 2>&1 | grep -c "(missing)" || echo 0' 2>/dev/null | tail -1)"
...
# F8: hk validate
echo "--- F8: hk Validate ---"
HK_RESULT="$(docker run --rm --platform "${PLATFORM}" "${IMAGE_REF}" bash -lc 'cd ~/.local/share/chezmoi && HK_FILE=hk.pkl hk validate >/dev/null 2>&1' && echo 'PASS' || echo 'FAIL')"
echo "hk validate: ${HK_RESULT}"

# F9: Bind mount performance
echo "--- F9: Bind Mount I/O ---"
BIND_WRITE_TIME=$(time_cmd "docker run --rm --platform ${PLATFORM} -v '${REPO_ROOT}:/workspace' ${IMAGE_REF} bash -c 'dd if=/dev/zero of=/workspace/.benchtemp bs=1M count=50 2>/dev/null'")
...
COMPUTE_TIME=$(time_cmd "docker run --rm --platform ${PLATFORM} ${IMAGE_REF} bash -c 'echo \"#include <stdio.h>
