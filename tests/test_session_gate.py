...
def _finalize_spec(repo: Path) -> Path:
    carrier = repo / "python" / "src" / "dotfiles_setup" / "sync.py"
    ...
    sentinel = json.dumps(
        {
            ...
            "prevention_id": "credential-launcher",
            "mutant": "credential-launcher-missing",
            "status": "ARMED",
            "target": "python/src/dotfiles_setup/sync.py",
            "before_sha256": "c" * 64,
            "after_sha256": "d" * 64,
...
def test_mutation_is_killed_when_no_registered_edit_occurs(
    ...
    repo = _repo(tmp_path)
    target = repo / "python" / "src" / "dotfiles_setup" / "sync.py"
