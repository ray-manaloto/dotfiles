...
def test_classifier_shaped_finds_the_three_real_classifiers() -> None:
    ...
    **4 hits, zero false positives**. The 2026-08-07 measurement was 3 across
    ...
    ⚠️ It also RETIRES a claim this gate's own prose used to make: that the
    hits were "both named `classify`". `edge_for` is not, and that is
    evidence FOR the predicate, not against it — `classifier_shaped` matches
    on the return annotation naming a module-local enum, never on the name.
    """
    found = {
        f"{path.name}:{fn}"
