...
@pytest.mark.parametrize("tool", ["mise", "chezmoi", "uv", "pixi", "claude", "gemini", "codex"])
def test_tool_reachable_in_login_shell(tool):
    ...
    assert result.returncode == 0, f"{tool} not found in login shell PATH. Stderr: {result.stderr}"
    
    # Verify it's using our managed paths (not system paths)
    # Note: claude is in ~/.local/bin, others are in shims
...
def test_tool_execution_in_login_shell(tool):
    ...
    assert result.returncode == 0, f"{tool} failed to execute in login shell. Stderr: {result.stderr}"
