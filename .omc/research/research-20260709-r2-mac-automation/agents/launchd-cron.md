...
- **osascript `display notification` is fragile from background contexts**: since Big Sur the notification is attributed to the calling process, which needs Notification permission; terminal apps often never appear in the Notifications pane until a notification is delivered — a chicken-and-egg that makes osascript notifications silently vanish from launchd jobs. Documented: <https://github.com/gsd-build/gsd-2/issues/2632>, <https://forum.latenightsw.com/t/trying-to-use-terminal-for-display-notification/5068>.
