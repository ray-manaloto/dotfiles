...
Domain: `.omc/research/research-20260709-r2-knowledge-base/`. Five angle
...
- **Anthropic's own guidance, plus two 2026 papers, converge on the
  ...
  `.omc/notepad.md` + `.claude/rules/*.md`) as the primitive, not a vector
...
- **Claude Code native memory surfaces**, mapped as KB candidates (all
  ...
  | OMC notepad + `project-memory.json` | As-you-go via MCP tools | worktree-local, `.omc/**` committable | No — session-scale, not corpus-scale; correct role is the working layer that *feeds* the corpus |
...
1. Run graphify over `docs/research/` + `.omc/research/`, **excluding
...
3. **At synthesis time** (cadence-gated, Layer 2 only): seed load is
   `INDEX.md` + the previous `GRAPH_REPORT.md` + `.omc/notepad.md` — a few
   ...
   (`omc-directory-conventions.md` rule 5).
...
1. **Corpus-boundary decision (needs Ray — flagged below, not resolved
   ...
   working files stay per-clone in `.omc/research/`, so the KB has one
...
1. **Corpus boundary**: should durable run outputs (`report.md`,
   ...
   tracked location, leaving `.omc/research/` purely as per-clone agent
...
4. **INDEX.md granularity**: one index for the whole corpus, or one per
   ...
   `.omc/research/INDEX.md`)? *Recommended: the split* — matches the
