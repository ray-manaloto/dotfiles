...
- **Skill already in Ray's toolbox**: the in-repo `graphify/skill.md` matches
  ...
  inventory (`.omc/research/research-20260709-r2-inventory/report.md:109-111`);
  the KB pilot must run on the Mac (skill absent in the remote container).
- **Corpus shape**: `.omc/research/**` + `docs/research/**` + `.claude/rules/`
...
- **Committable artifacts**: relative-path graph.json + union merge driver +
  the `.omc/**`-is-committable fact (inventory report :112-116) mean
...
- [ray-manaloto/dotfiles](https://github.com/ray-manaloto/dotfiles) — grounding baseline read from the working tree (`.omc/research/research-20260709-r2-inventory/report.md`).
