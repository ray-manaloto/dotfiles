...
def _add_docker_subcommands(
    ...
    verify_ssh_parser = docker_subparsers.add_parser(
        "verify-ssh-inbound",
        help="Verify R1 host-to-container SSH with the resolved host agent",
    )
    verify_ssh_parser.add_argument("--port", type=int, required=True)
    verify_ssh_parser.add_argument("--user", required=True)
...
def handle_docker(
    ...
    elif args.docker_command == "verify-ssh-inbound":
        docker_manager.verify_ssh_inbound(args.port, args.user)
