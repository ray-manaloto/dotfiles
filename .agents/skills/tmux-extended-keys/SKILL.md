...
Inside tmux, any TUI (Codex, vim, emacs, nvim, helix) sees
`Shift+Enter` as a plain `\r`, identical to `Enter`. In Codex
this means Shift+Enter submits the prompt instead of adding a newline.
`/terminal-setup` inside Codex refuses to run under tmux and
...
- Codex's `/terminal-setup` errors with
...
1. **Edit tmux config** (`~/.tmux.conf` or `~/.config/tmux/tmux.conf`,
   ...
   ```tmux
   # Forward CSI-u extended-key sequences so TUIs like Codex,
...
4. **Test in the target TUI.** In Codex: type any prompt,
...
# distinguishable in TUIs like Codex / vim / emacs.
set -s extended-keys on
set -as terminal-features 'xterm*:extkeys'
```
...
- Codex `/terminal-setup` (refuses inside tmux)
- `feedback_omc_phantom_teammatemode.md` — another "looks configured
  but isn't wired" gotcha in the same Codex + tmux stack
