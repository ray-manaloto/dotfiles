...
| `FAIL converge: dev-rebuild rc=…` | Lifecycle rebuild failed | Read the streamed devcontainer-cli output; `getaddrinfo ENOTFOUND ghcr.io` = transient DNS, retry once per `.Codex/rules/persistence-gate-retry.md` |
...
- `.Codex/skills/devcontainer-workflow/SKILL.md` — up/down/smoke basics.
- `.Codex/rules/verify-before-advancing.md` — when a synced container is
  a valid validation environment.
- `.Codex/rules/persistence-gate-retry.md` — transient-DNS retry heuristic.
