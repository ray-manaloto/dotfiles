...
**Correction — 2026-09-17 (#1183):** That conclusion holds only while the mount
root's owner is stable. Docker Desktop's virtiofs intermittently (single-sample
flickers under load, measured live the same day) reports the workspace mount root as `0:0` while `.git` and its files remain
`1000:1000`, which makes Git return rc=128 and hk/libgit2 report `Owner (-36)`.
The durable fix is one workspace-scoped `safe.directory` entry rendered by the
chezmoi-managed global gitconfig; the eight unrelated mde entries remain inert
and dropped.
