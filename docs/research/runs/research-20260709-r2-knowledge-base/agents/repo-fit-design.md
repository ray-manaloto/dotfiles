...
`docs/research/runs/research-20260709-r2-inventory/report.md` plus the three
...
**Layer 1 — `docs/research/runs/**` (working research, per-clone-ignored on the Mac,
committable here).** This fresh clone holds only the current R2 round:
**36 markdown files** across 8 run directories (Glob `docs/research/runs/**/*.md`,
...
**Layer 4 — OMC state:** `.agent/notepad.md`, `.agent/project-memory.json`
(per `agent-artifact-conventions.md`).
...
**Committability nuance that shapes every candidate:** `.agent/**` is NOT
gitignored in the repo (only `.agent/state/`; inventory report:112-116), so
...
location**: the Mac's historical `.agent/research` (e.g. the 104-agent run) is
...
1. **`docs/research/INDEX.md`** (and a per-clone `docs/research/runs/INDEX.md`),
...
`docs/research/runs/` with `--update` (incremental SHA256 caching re-extracts only
...
1. **Corpus boundary decision** (needs Ray, per `clarify-before-acting.md`):
   ...
   `docs/research/runs/` (committable-on-branch). This gives the KB one canonical,
...
`.agent/notepad.md` (a few hundred lines total, versus dozens of full reports),
...
- **Corpus-boundary decision is unresolved** (F1): index coverage of the
  Mac-only historical `.agent/research` runs depends on step 1 of the
