...
(`docs/research/runs/**`, `docs/research/**`) currently lacks its own llms.txt-shaped
...
- This validates the repo's existing stack (`.agent/notepad.md`,
...
2. **Give the corpus an llms.txt-shaped index**: one curated markdown file per
   corpus area (`docs/research/runs/`, `docs/research/`) with one line per artifact
