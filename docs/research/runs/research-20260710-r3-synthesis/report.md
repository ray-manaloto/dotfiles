...
|---|---|---|
| **A — mise dotfiles vs chezmoi** | ⚠️ **SUPERSEDED 2026-08-01 by [#448](https://github.com/ray-manaloto/dotfiles/issues/448)** — was "NEITHER (keep chezmoi)" | Current verdict: **mise `bootstrap dotfiles` is the destination; chezmoi holds through the #431 takeover** — deferred on blast radius and maturity, **not** capability. Re-measured on mise 2026.8.0: `promptBoolOnce` is superseded by #439's OS-branch decision, `copy` preserves 0600, `is exists` replaces `stat`, and platform environments replace the OS gate (so the `os_family()` refutation in row 6 is right but moot). Receipt: `docs/receipts/448.md`. |
