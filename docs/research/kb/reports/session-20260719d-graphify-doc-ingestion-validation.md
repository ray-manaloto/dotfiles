...
(`22it2mDFiF8.md`, "Graphify + Obsidian") and one dense technical report
...
3. If doc retrieval is needed now, prefer a method that preserves the source
   ...
   `docs/research/kb/`; plain retrieval over them loses nothing that the graph adds).
...
INGEST_TIMEOUT=900 INGEST_LOG=/tmp/kb-probe.log \
  ingest_kb.sh docs/research/kb/probe docs/research/kb/graphs/probe qwen3:14b
```
