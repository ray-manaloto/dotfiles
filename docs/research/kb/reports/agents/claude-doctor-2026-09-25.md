# Built-in `/doctor` — 2026-09-25 (task_plan.md § "2026-09-25 step 0", item 1)

Run in session "dotfiles-20260925.000" (Claude Code 2.1.283, Opus 5.5) after Ray upgraded
2.1.282 → 2.1.283 with `claude install latest`. `/doctor` is the built-in health-check prompt:
...
| 5 | 1 skills | 10 user skills `source-command-memory-index-curation-skill-<8 hex>` are symlinks (created 2026-09-11) into `~/.agents/skills/`, all with the description `"Description delivered above."`. Lifetime uses 0, window hits 0; about 0.9k est. listing tokens | **FIXED** (user scope): `skillOverrides` `"off"` ×10 in `~/.claude/settings.json` (backup: `~/.claude/settings.json.bak-doctor-2026-09-25`). Undo: delete the 10 keys. What created them is not established |
| 6 | 1 skills | `herdr` (user): 0 uses. It fires only on an explicit request | **ACCEPTED** (Claude, at Ray's "clean up everything" gate): Phase 9 (herdr) is queued in `task_plan.md` |
| 7 | 1 skills | 25 project skills have 0 lifetime uses and 0 window hits (e.g. `lock-shared`, `reap`, `pin-parity`, `plugin-removal`) | **ACCEPTED** (same gate): they are repo runbooks triggered by rare events, and turning them off locally would hide them exactly when their trigger fires |
