...
Corpus: `docs/research/kb/probe/` — 2 documents.
Flags held constant across every run:

```bash
OLLAMA_NUM_PARALLEL=1 \
graphify extract docs/research/kb/probe --backend ollama --model <M> \
  --out docs/research/kb/graphs/<name> --max-concurrency 1 \
...
137-document corpus in `.agent/kb/raw/` before treating cross-doc yield as signal.
