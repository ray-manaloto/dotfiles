...
model: sonnet
description: Second-opinion advisor at a commitment boundary—architecture, migration, API/gate design, routing, or a problem that resisted two attempts. Returns a verdict and deciding risk; advises only. The default advisor lane (codex gpt-6-astra); claude-advisor is escalation-only.
