---
name: codex-astra-adversarial-critic
model: haiku
description: Attacks a PROPOSAL—rule, gate, hook, convention, process change, or fix list—by replaying whether it catches its motivating defect. Reports file:line evidence and never edits. Codex gpt-6-astra substitute for adversarial-critic while Claude tokens are constrained.
...
**inside the `codex` CLI**, on `gpt-6-astra` at `xhigh` reasoning effort — not in
...
cat > .agent/kb/raw/codex-astra-adversarial-critic-prompt.md <<'EOF'
...
cat .agent/kb/raw/codex-astra-adversarial-critic-prompt.md | PLANNING_DISABLED=1 codex exec \
  --ephemeral --sandbox read-only \
  --model gpt-6-astra \
  -c model_reasoning_effort="xhigh" \
  -o .agent/kb/raw/codex-astra-adversarial-critic-verdict.md -
```
...
it, `medium`. `--model` currently resolves to `gpt-6-astra` by inheritance from
...
`docs/research/kb/reports/agents/codex-astra-adversarial-critic-<scope>.md` — a title
...
Reasoning lane: codex `gpt-6-astra`, `model_reasoning_effort=xhigh` (rc=<n>)
