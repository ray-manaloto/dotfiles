---
name: plugin-removal
description: Remove a Claude Code or codex plugin through the checked inventory, dry-run plan, native CLIs, verification, and per-repository PR workflow. Use whenever a plugin, marketplace, plugin cache, or trusted plugin hook must be removed. The procedure is dry-run-first and keeps worktrees report-only.
---
